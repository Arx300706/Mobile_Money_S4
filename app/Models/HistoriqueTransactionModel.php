...
class HistoriqueTransactionModel extends Model
{
    ...
    {
        return $this->select('historique_transaction.*, type_operations.nom AS type_operation, tr.id_compte_client, tr.id_compte_destinataire, client_source.telephone AS telephone_source, client_destinataire.telephone AS telephone_destinataire')
    ...
    public function findByCompte(int $compteId): array
    {
        return $this->withDetails()
            ->groupStart()
                ->groupStart()
                    ->where('tr.id_compte_destinataire', null)
                    ->where('tr.id_compte_client', $compteId)
                ->groupEnd()
                ->orGroupStart()
                    ->where('tr.id_compte_destinataire IS NOT NULL')
                    ->where('tr.id_compte_client', $compteId)
                    ->where('historique_transaction.solde_apres < historique_transaction.solde_avant')
                ->groupEnd()
                ->orGroupStart()
                    ->where('tr.id_compte_destinataire', $compteId)
                    ->where('historique_transaction.solde_apres > historique_transaction.solde_avant')
                ->groupEnd()
            ->groupEnd()
